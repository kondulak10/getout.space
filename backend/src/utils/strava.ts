...
export async function refreshStravaToken(user: IUser): Promise<{
	...
	if (!response.ok) {
		...
		console.error('❌ Token refresh failed!', {
			...
			refreshTokenPrefix:
				user.refreshToken.length >= 10 ? user.refreshToken.substring(0, 10) + '...' : '[short]',
			refreshTokenLength: user.refreshToken.length,
		});

		if (response.status === 401) {
			throw new Error(
				`Token refresh failed with 401 - refresh token may have been already used or user revoked access (user: ${user.stravaProfile.firstname})`
			);
		}
